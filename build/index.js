...
var tma_default = `<!DOCTYPE html>\r
<html lang="zh">\r
\r
<head>\r
  <meta charset="UTF-8">\r
  <meta content="width=device-width, initial-scale=1.0" name="viewport">\r
  <title>My-email</title>\r
  <script src="https://telegram.org/js/telegram-web-app.js"><\/script>\r
  <script src="https://unpkg.com/vue@3/dist/vue.global.prod.js"><\/script>\r
  <link href="https://unpkg.com/boltcss/bolt.min.css" rel="stylesheet">\r
  <style>\r
    body {\r
      max-width: 600px;\r
      margin: 0 auto;\r
...
<body>\r
  <div id="app">\r
    <div v-if="func === 'list'">\r
      <h3 class="growContainer">\r
        List Mode\r
        <select class="growItem" style="margin-left: 10px;" v-model="mode">\r
          <option value="block">Block list</option>\r
          <option value="white">White list</option>\r
          <option value="test">Test address</option>\r
        </select>\r
      </h3>\r
      <p v-if="tipMessage">{{ tipMessage }}</p>\r
      <table>\r
        <thead>\r
          <tr>\r
            <th>address</th>\r
            <th>{{ mode === 'test' ? 'type' : 'action' }}</th>\r
          </tr>\r
        </thead>\r
        <tbody v-if="mode!=='test'">\r
          <tr>\r
            <td class="growContainer">\r
              <input :placeholder="inputPlaceholder" class="growItem" v-model="inputAddress">\r
            </td>\r
            <td>\r
              <button @click="addAddress">Add</button>\r
            </td>\r
          </tr>\r
          <tr :key="index" v-for="(address, index) in addresses">\r
            <td>{{ address }}</td>\r
            <td>\r
              <button @click="removeAddress(index)">Delete</button>\r
            </td>\r
          </tr>\r
        </tbody>\r
        <tbody v-else>\r
          <tr>\r
            ...
            <td>\r
              <button @click="testAddress">Test</button>\r
            </td>\r
          </tr>\r
          <tr :key="index" v-for="(item, index) in addresses">\r
            <td>{{ item.address }}</td>\r
            <td>{{ item.result }}</td>\r
          </tr>\r
        </tbody>\r
      </table>\r
    </div>\r
    <div v-if="func === 'sendmail'">\r
      <h3>Send Mail</h3>\r
      <p v-if="tipMessage">{{ tipMessage }}</p>\r
      <table>\r
        <tbody>\r
          <tr>\r
            <td style="width: 10px;">From</td>\r
            <td>\r
              <input v-model="sendMail.from" class="growItem">\r
            </td>\r
          </tr>\r
          <tr>\r
            <td>To</td>\r
            <td>\r
              <input v-model="sendMail.to" class="growItem">\r
            </td>\r
          </tr>\r
          <tr>\r
            <td>Subject</td>\r
            <td>\r
              <input v-model="sendMail.subject" class="growItem">\r
            </td>\r
          </tr>\r
          <tr>\r
            <td>Text</td>\r
            <td>\r
              <textarea v-model="sendMail.text" class="growItem"></textarea>\r
            </td>\r
          </tr>\r
          <tr>\r
            <td colspan="2">\r
...
\r
  <script>\r
    const { createApp, computed, ref, onMounted, watch } = Vue;\r
\r
\r
    class Client {\r
\r
      constructor(tma) {\r
        this.tma = tma;\r
        this.addAddress = this.addAddress.bind(this);\r
        this.removeAddress = this.removeAddress.bind(this);\r
        this.loadAddress = this.loadAddress.bind(this);\r
      }\r
\r
      async request(path, method, body) {\r
        const res = await fetch(path, {\r
          method: method,\r
          headers: {\r
            'Content-Type': 'application/json',\r
            'Authorization': \`tma \${this.tma}\`,\r
          },\r
          body: JSON.stringify(body),\r
        }).then(response => response.json());\r
        if (res.error) {\r
          throw new Error(res.error);\r
        }\r
        return res;\r
      }\r
\r
      async addAddress(address, type) {\r
        return this.request('/api/address/add', 'POST', {\r
          address,\r
          type,\r
        });\r
      }\r
\r
      async removeAddress(address, type) {\r
        return this.request('/api/address/remove', 'POST', {\r
          address,\r
          type,\r
        });\r
      }\r
\r
      async loadAddress(tma) {\r
        return this.request('/api/address/list', 'GET');\r
      }\r
    }\r
\r
    createApp({\r
...
\r
        const blockList = ref([]);\r
        const whiteList = ref([]);\r
        const testList = ref([]);\r
        const tipMessage = ref('');\r
        const inputAddress = ref('');\r
\r
        const sendMail = ref({\r
          from: '',\r
          to: '',\r
          subject: '',\r
          text: '',\r
...
\r
        const func = ref(urlParams.get('func') || 'list');\r
        const mode = ref(urlParams.get('mode') || 'block');\r
        const client = new Client('');\r
\r
        const addresses = computed(() => {\r
          switch (mode.value) {\r
            case 'block':\r
              return blockList.value;\r
            case 'white':\r
              return whiteList.value;\r
            case 'test':\r
              return testList.value;\r
          }\r
        });\r
\r
        const inputPlaceholder = computed(() => {\r
          switch (mode.value) {\r
            case 'block':\r
              return 'New block address regex';\r
            case 'white':\r
              return 'New white address regex';\r
            case 'test':\r
              return 'Test address';\r
          }\r
        });\r
\r
        const addAddress = async () => {\r
          try {\r
            if (!inputAddress.value) {\r
              return;\r
            }\r
            const address = inputAddress.value.trim();\r
            await client.addAddress(address, mode.value);\r
            inputAddress.value = '';\r
            switch (mode.value) {\r
              case 'block':\r
                blockList.value.push(address);\r
                break;\r
              case 'white':\r
                whiteList.value.push(address);\r
                break;\r
              case 'test':\r
                break;\r
            }\r
            tipMessage.value = null;\r
          } catch (error) {\r
            tipMessage.value = \`ERROR: \` + error;\r
          }\r
        };\r
\r
        const removeAddress = async (index) => {\r
          try {\r
            const address = addresses.value[index];\r
            await client.removeAddress(address, mode.value);\r
            switch (mode.value) {\r
              case 'block':\r
                blockList.value.splice(index, 1);\r
                break;\r
              case 'white':\r
                whiteList.value.splice(index, 1);\r
                break;\r
              case 'test':\r
                break;\r
            }\r
...
\r
        const testAddressWithPattern = (pattern, address) => {\r
          if (pattern.toLowerCase() === address.toLowerCase()) {\r
            return true;\r
          }\r
          try {\r
            const regex = new RegExp(pattern, 'i');\r
            return !!regex.test(address);\r
          } catch (e) {\r
            return false;\r
          }\r
        };\r
\r
        const testAddress = () => {\r
          const test = [];\r
          const address = inputAddress.value.trim();\r
          for (const pattern of blockList.value) {\r
            if (testAddressWithPattern(pattern, address)) {\r
              test.push({\r
                address: pattern,\r
                result: 'block',\r
              });\r
          ...
          for (const pattern of whiteList.value) {\r
            if (testAddressWithPattern(pattern, address)) {\r
              test.push({\r
                address: pattern,\r
                result: 'white',\r
...
\r
        onMounted(async () => {\r
          try {\r
            client.tma = window.Telegram.WebApp.initData;\r
            const { block, white } = await client.loadAddress();\r
            blockList.value = block;\r
            whiteList.value = white;\r
          } catch (error) {\r
            tipMessage.value = \`ERROR: \` + error.message;\r
          }\r
        });\r
\r
        return {\r
...
async function parseEmail(message, maxSize, maxSizePolicy, useEmlHeaders = false) {
  ...
  try {
    ...
    cache.fromName = email.from?.name || void 0;
...
[Truncated] The original size of the email was ${message.rawSize} bytes, which exceeds the maximum size of ${maxSize} bytes.`;
    ...
    cache.body = cache.text?.slice(0, 2e3) || "(empty body)";
...
function buildHtml(body) {
  const lines = body.split("\n").map((l3) => l3 === "" ? "<br>" : `<p style="margin:0 0 8px">${l3}</p>`).join("\n");
  return `<!DOCTYPE html>
<html lang="en">
...
async function replyToEmail(token2, email, message) {
  await sendEmail(token2, email.to, {
    to: email.from,
...
async function sendEmail(token2, from, opts) {
  const payload = {
    from,
    to: [opts.to],
...
// src/telegram/flow.ts
async function handleFlow(message, env) {
  const chatId = message.chat.id;
  const text = message.text?.trim() || "";
  const token2 = env.TELEGRAM_BOT_TOKEN;
  ...
  if (text.toLowerCase().startsWith("/forward")) {
    ...
    await api.sendMessage({
      chat_id: chatId,
      text: `\u{1F4E8} <b>Forwarding #${id}</b>
  ...
  if (draft && text !== "") {
    switch (draft.step) {
      ...
      case "send_body":
        ...
        await api.sendMessage({
          chat_id: chatId,
          text: `\u{1F4E4} <b>Preview \u2014 New Email</b>
...
<code>\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500</code>`,
      ...
      case "forward_to": {
        ...
        await api.sendMessage({
          chat_id: chatId,
          text: `\u{1F4E4} <b>Preview \u2014 Forward</b>
...
async function handleFlowCallback(callback, env) {
...
<b>Subject:</b> Fwd: ${draft.email.subject}`,
      ...
      await api.sendMessage({ chat_id: chatId, text: `\u274C Failed to send: ${e2.message}` });
    }
    return true;
  }
  return false;
  ...
  const renderHandlerBuilder = (render2) => {
    return async (arg2) => {
      ...
      const params = {
        ...
        ...req,
        parse_mode: "HTML"
        // Ensure HTML parsing is allowed for the updated full experience
      };
...
// src/handler/fetch/dashboard.ts
function registerDashboardRoutes(router, env, authMiddleware) {
  const { EMAIL_STORE } = env;
  const dao = new Dao(EMAIL_STORE);
  router.get("/api/emails", authMiddleware, async () => {
  ...
  router.get("/api/emails/:id", authMiddleware, async (req) => {
  ...
  router.delete("/api/emails/:id", authMiddleware, async (req) => {
    const id = req.params.id;
    await EMAIL_STORE.delete(id);
    return { success: true };
  });
  router.post("/api/emails/send", authMiddleware, async (req) => {
    const body = await req.json();
...
function createTmaAuthMiddleware(env) {
  ...
  return async (req) => {
    ...
    try {
      await validate2(authData, TELEGRAM_BOT_TOKEN, {
        expiresIn: 3600
      });
...
async function emailHandler(message, env) {
  ...
  try {
    ...
    if (!status.telegram && !blockTelegram) {
      const ttl = Number.parseInt(MAIL_TTL, 10) || 60 * 60 * 24 * 30;
      ...
      if (attachments && attachments.length > 0) {
        ...
        for (const att of attachments) {
          if (att.content) {
            for (const id2 of TELEGRAM_CHAT_ID.split(",")) {
              try {
                await api.sendDocument({
                  chat_id: id2,
                  document: new File([att.content], att.filename || "attachment", { type: att.mimeType || "application/octet-stream" }),
                  caption: `\u{1F4CE} Attachment from email #${mail.id}: ${att.filename || "file"}`
                });
              } catch (err) {
